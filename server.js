...
app.post('/', async (req, res) => {
  try {
    ...
    await pool.execute("INSERT INTO users(name,description) VALUES (?,?) ", [name, description])
    res.json({
...
app.put('/:id', async (req, res) => {
  try {
    ...
    if (row.length === 0) {
      ...
      await pool.execute("UPDATE users set name=? , description=? where id=?", [name || row[0].name, description || row[0].description, id])
      res.json({
